...
public class EtiquetteparametragedemandeResource {
  ...
  @PostMapping("/etiquetteparametragedemandes")
  public ResponseEntity<EtiquetteparametragedemandeDTO> createEtiquetteparametragedemande(@Valid @RequestBody EtiquetteparametragedemandeDTO etiquetteparametragedemandeDTO, BindingResult bindingResult) throws URISyntaxException, MethodArgumentNotValidException {
    log.debug("REST request to save Etiquetteparametragedemande : {}", etiquetteparametragedemandeDTO);
    if (bindingResult.hasErrors()) {
      throw new MethodArgumentNotValidException(null, bindingResult);
    }
    if ( etiquetteparametragedemandeDTO.getCode() != null) {
      bindingResult.addError( new FieldError("EtiquetteparametragedemandeDTO","code","POST method does not accepte "+ENTITY_NAME+" with code"));
      throw new MethodArgumentNotValidException(null, bindingResult);
    }
    EtiquetteparametragedemandeDTO result = etiquetteparametragedemandeService.save(etiquetteparametragedemandeDTO);
    return ResponseEntity.created( new URI("/api/etiquetteparametragedemandes/"+ result.getCode())).body(result);
  }

  @PutMapping("/etiquetteparametragedemandes")
  public ResponseEntity<EtiquetteparametragedemandeDTO> updateEtiquetteparametragedemande(@Valid @RequestBody EtiquetteparametragedemandeDTO etiquetteparametragedemandeDTO, BindingResult bindingResult) throws MethodArgumentNotValidException {
    log.debug("Request to update Etiquetteparametragedemande: {}",etiquetteparametragedemandeDTO);
    if (bindingResult.hasErrors()) {
      throw new MethodArgumentNotValidException(null, bindingResult);
    }
    if ( etiquetteparametragedemandeDTO.getCode() == null) {
      bindingResult.addError( new FieldError("EtiquetteparametragedemandeDTO","code","PUT method does not accepte "+ENTITY_NAME+" with code"));
      throw new MethodArgumentNotValidException(null, bindingResult);
    }
    EtiquetteparametragedemandeDTO result =etiquetteparametragedemandeService.update(etiquetteparametragedemandeDTO);
